...
public class Cat implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class CatPet implements Sayable{
            ...
            public String say() {
                if (i < 1){
                    String formatedSleep = String.format("%s спит.", Cat.this.name);
                    return formatedSleep;
                }
                else if (i >= 1){
                    ...
                    for (int j = 0; j < i; j++)
                        sb.append("я");
                    sb.append("у");
                    String formatSayMeow = String.format("%s говорит %s!", Cat.this.name, sb.toString());
                    return formatSayMeow;
                }
                return null;
            }
        }
